...
-- Begin add @lg "TINYINT" support 
function convert.tinyint(v, driver)
   return tonumber(v)
end
-- End add @lg "TINYINT" support
...
-- Modify @lg
-- Sqlite 3 support: new type "NUMERIC"
-- However, don't use the new type in DB
...
--Begin agg @lg support the value 1 for "TRUE" 
    return v == "t" or  tonumber(v) == 1
--End agg @lg support the value 1 for "TRUE" 
...
-- Begin add @lg "BOOL" support 
function convert.bool(v, driver)
  if driver == "sqlite3" then
     return v == "t" or  tonumber(v) == 1
  elseif driver == "mysql" then
    return tonumber(v) == 1
...
-- End add @lg "BOOL" support

-- Begin add @lg "TIMESTAMP" support 
function convert.timestamp(v, driver)
  if driver == "sqlite3" then
    return tonumber(v)
  elseif driver == "mysql" then
    return tonumber(v)
...
--Begin add @lg
function escape.bool(v, driver)
  -- if v then
  if v== true or v == "t" or  tonumber(v) == 1 then
    --if driver == "sqlite3" then return "'t'" else return tostring(v) end
	if driver == "sqlite3" then return tostring(1) else return tostring(v) end
  else
    --if driver == "sqlite3" then return "'f'" else return tostring(v) end
	if driver == "sqlite3" then return tostring(0) else return tostring(v) end
  end
end

function escape.tinyint(v)
  return tostring(v)
end

function escape.timestamp(v)
  return escape.integer(v)
end
--End add @lg
